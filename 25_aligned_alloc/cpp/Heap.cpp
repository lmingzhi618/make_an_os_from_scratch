...
void Free(void* addr) {
    MemorySegmentHeader* curSeg;

    AlignedMemorySegmentHeader* AMSH = (AlignedMemorySegmentHeader*)addr - 1;
    if (AMSH->IsAligned) {
        curSeg = (MemorySegmentHeader*)(uint_64)AMSH->MemorySegmentHeaderAddr;
    } else {
        curSeg = ((MemorySegmentHeader*)addr) - 1;
    }
...
void* Realloc(void* addr, uint_64 newSize) {
    MemorySegmentHeader* oldSegHeader;

    AlignedMemorySegmentHeader* AMSH = (AlignedMemorySegmentHeader*)addr - 1;
    if (AMSH->IsAligned) {
        oldSegHeader = (MemorySegmentHeader*)(uint_64)AMSH->MemorySegmentHeaderAddr;
    } else {
        oldSegHeader = ((MemorySegmentHeader*)addr) - 1;
    }
...
void* AlignedAlloc(uint_64 alignment, uint_64 size) {
    ...
    uint_64 addr = (uint_64)mallocVal;

    uint_64 remainder = addr % alignment;
    addr -= remainder;
    if (remainder) {
        addr += alignment;
        AlignedMemorySegmentHeader* AMSH = (AlignedMemorySegmentHeader*)addr - 1;
        AMSH->IsAligned = true;
        AMSH->MemorySegmentHeaderAddr = (uint_64)mallocVal - sizeof(MemorySegmentHeader);
    }
    return (void*)addr;
}
